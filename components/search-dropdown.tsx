...
export default function SearchDropdown({ 
  ...
  const [recentSearches, setRecentSearches] = useState<Array<{name: string, id?: string, image?: string | Media}>>([])
  ...
  useEffect(() => {
    const loadTrendingGames = async () => {
      ...
      if (trendingGamesCache && (now - trendingGamesCacheTimestamp) < TRENDING_CACHE_DURATION) {
  ...
  const handleSearch = (searchTerm: string, gameId?: string, gameImage?: string | Media) => {
    setQuery(searchTerm)
    setIsOpen(false)
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (selectedIndex >= 0 && results[selectedIndex]) {
          handleSearch(results[selectedIndex].name, results[selectedIndex].id, results[selectedIndex].image)
        } else if (query.trim()) {
          handleSearch(query)
        }
        break
  ...
  const highlightMatch = (text: string, searchQuery: string) => {
    ...
    return parts.map((part, i) => 
      regex.test(part) ? (
        <span key={i} className="bg-yellow-400/20 text-yellow-300 font-medium">{part}</span>
      ) : part
    )
  }
  ...
  return (
    <div ref={searchRef} className={cn("relative", className)}>
      {/* Search Input */}
      <div className="relative flex items-center bg-gray-800 rounded-lg px-3 py-2 border border-gray-700 focus-within:border-gray-600 transition-colors">
      ...
      {(showSuggestions || showResults || showNoResults) && (
        <div className={cn(
          "absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto",
                  ...
                  <div className="space-y-1">
                    {trendingGames.map((game) => (
                      <button
                        ...
                        className="w-full text-left p-2 text-sm text-gray-300 hover:text-white hover:bg-gray-700 rounded transition-colors flex items-center gap-3"
                      >
                        <div className="w-8 h-8 rounded-md overflow-hidden flex-shrink-0 bg-gray-600">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {recentSearches.map((item, index) => (
                      <button
                        key={`${item.name}-${index}`}
                        onClick={() => handleSearch(item.name, item.id, item.image)}
                        className="w-full text-left p-2 text-sm text-gray-300 hover:text-white hover:bg-gray-700 rounded transition-colors flex items-center gap-3"
                      >
                        <div className="w-8 h-8 rounded-md overflow-hidden flex-shrink-0 bg-gray-600">
